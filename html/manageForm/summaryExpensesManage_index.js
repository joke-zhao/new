define(function (require, exports, module) {
  ...
  var main = new Vue({
    ...
    el: "#main",
    components: {vant},
    template: _g.getTemplate("manageForm/summaryExpensesManage_index_view"),
    data: {
      deptId: "",
      companyName: "",
      dialogShow: false,
    ...
    methods: {
      ...
      selectAmtIncomeByMonth() {
        var self = this;
        var url = "api/control/erpcollect/selectCollectFeesList";
        Http.ajax({
          data: {
            "data": {
              //改
            ...
            if(ret.code==200){
              ...
              console.log(incomeList)
                for (let i = 0; i < incomeList.length; i++) {
                  incomeList[i].thisMonth = (incomeList[i].thisMonth || 0)
                      .toFixed(2)
                      .replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, "$&,");
                  incomeList[i].lastMonth = (incomeList[i].lastMonth || 0)
                      .toFixed(2)
                      .replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, "$&,");
                  incomeList[i].thisYear = (incomeList[i].thisYear || 0)
                      .toFixed(2)
                      .replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, "$&,");
                }
            }else{
        ...
        if(this.incomeMonthList.length === 0){
          ...
          var option = {
            data: {
              createHeader: true,
              tableName:'销管研制费用汇总表',
              title:main.value +'~'+ main.companyName + '~销管研制费用汇总表',
