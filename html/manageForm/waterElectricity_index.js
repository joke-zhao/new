define(function (require, exports, module) {
    ...
    var main = new Vue({
        el: "#main",
        components: { vant },
        template: _g.getTemplate("manageForm/waterElectricity_index_view"),
        data: {
            showShare: false,
            options: [
        ...
        methods: {
            ...
            queryPAL() {
                ...
                var self = this;
                //根据部门id和时间
                //var url = "api/control/query/WaterAndElectricity?dateE=" + this.endTime + "&dateS=" + this.startTime + "&wh=" + "C003"
                var url = "api/control/query/WaterAndElectricity?dateE=" + this.endTime + "&dateS=" + this.startTime + "&wh=" + this.deptId
                Http.ajax({
                    data: {},
                    isFile: false,
                    isJson: false,
                    ...
                    success: function (ret) {
                        ...
                        if (ret) {
                            ...
                            for (var i of self.palList) {
                                self.sWater = i.water + self.sWater;
                                self.sWaterNumber = i.waterNumber + self.sWaterNumber;
                                self.sElectricity = i.electricity + self.sElectricity;
                                self.sElectricityNumber = i.electricityNumber + self.sElectricityNumber;
                                i.water = (+i.water || 0)
                                    .toFixed(2)
                                    .replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, "$&,");
                                i.electricity = (+i.electricity || 0)
                                    .toFixed(2)
                                    .replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, "$&,");
                            }
                        }
                        ...
                        for (var i of ret.data) {
                            switch (i.prdName.trim()) {
                                case "水费":
                                    X1.push(i.water);
                                    X2.push(i.waterNumber);
                                    break;
                                case "电费":
                                    X3.push(i.electricity);
                                    X4.push(i.electricityNumber);
                                    break;
                            }
                        }
                        ...
                        var option = {
                            ...
                            color: ["#24A9DE", "#99E187", "#403EEE", "#AD6FFE"],
                            legend: {
                                icon: "rect",
                                x: "center",
                                y: "bottom",
            ...
            exportExcel() {
                ...
                if (this.initList.length === 0) {
                    ...
                    this.palList.push({
                        date: '合计',
                        water: this.sWater,
                        waterNumber: this.sWaterNumber,
                        electricity: this.sElectricity,
                        electricityNumber: this.sElectricityNumber
                    })
                    ...
                    var option = {
                        data: {
                            createHeader: true,
                            tableName: '水电表',
                            title: this.startTime + "到" + this.endTime + '-' + _g.getLS('companyName') + '-水电表'+ '-币种：（CNY）',
                            list: this.palList,
                            sheetName: "表格1",
                            fileName: "excelForm",
                        },
